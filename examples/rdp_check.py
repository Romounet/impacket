...
if __name__ == '__main__':
    ...
    def check_rdp(host, username, password, domain, hashes = None):
       ...
       tls.do_handshake()
       master_key = str(tls.master_key()).encode('hex')
       client_random = str(tls.client_random()).encode('hex')
       print ' '.join(['CLIENT_RANDOM', client_random, master_key])
       #get_ssl_master_key(tls)

       # If you want to use Python internal ssl, uncomment this and comment 
       # the previous lines
       #import ssl
       #tls = ssl.wrap_socket(s, ssl_version=ssl.PROTOCOL_TLSv1_2)
       ...
       auth = ntlm.getNTLMSSPType1(workstation='DESKTOP-6BGN7C0',domain='DESKTOP-6BGN7C0', signingRequired=True, use_ntlmv2 = False)
       ...
       #ntlmChallenge.fromString(ts_request['NegoData'])
       #print ntlmChallenge['TargetInfoFields']
       print buff.encode('hex')
       


       type3, exportedSessionKey = ntlm.getNTLMSSPType3(auth, ts_request['NegoData'], username, password, 'DESKTOP-6BGN7C0', lmhash, nthash, use_ntlmv2 = True)
